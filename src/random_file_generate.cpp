...
void usage(const char* arg0){
    fprintf(stderr, "usage: %s [OPTIONS] <output_datafile> <size>\n"
                    ...
                    "K:KB(2^10 bytes)\nM:MB(2^20 bytes)\nG:GB(2^30 bytes)\nT:Bad file descriptorTB(2^40 bytes)\n"
            , arg0);
}
...
int main(int argc, char* argv[]){
...

    {
        ...
        switch(*end){
            case '\0': break;
            case 'k': case 'K': nbytes *= 1024ul;
            case 'm': case 'M': nbytes *= 1048576ul;
            case 'g': case 'G': nbytes *= 1073741824ul;
            case 't': case 'T': nbytes *= 1099511627776ul;
